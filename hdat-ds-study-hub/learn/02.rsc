...
13:T2122,
<span class="section-anchor" id="21" aria-hidden="true"></span>

<h3 id="22">2.2 시험 코드의 네 가지 객체</h3>
<div class="table-scroll" role="region" tabindex="0" aria-label="2강 표 1, 좌우로 스크롤할 수 있습니다"><table data-table="1">
<thead>
<tr>
<th>객체</th>
<th>강점</th>
...
<td>수치 연산이 느리고 broadcasting 없음</td>
</tr>
<tr>
...
<td>열 이름·혼합 dtype</td>
<td><code>x.shape</code>, <code>x.dtypes</code></td>
<td>문자열·결측 혼합 시 object/string 함정</td>
</tr>
<tr>
<td><code>torch.Tensor</code></td>
<td>autograd·GPU·신경망</td>
<td><code>x.shape</code>, <code>x.dtype</code>, <code>x.device</code></td>
<td>dtype/device/축 순서를 맞춰야 함</td>
</tr>
</tbody>
</table></div>
<p>실전에서 가장 먼저 출력할 것은 값 전체가 아니라 다음 다섯 가지다.</p>
<div class="code-shell"><div class="code-toolbar"><span>PYTHON</span><button type="button" data-copy-target="code-02-1">복사</button></div><pre id="code-02-1" tabindex="0" aria-label="2강 Python 코드 1"><code class="language-python">def inspect(name, x):
    print(name)
    print("type:", type(x))
...
<h3 id="23">2.3 축을 문장으로 읽기</h3>
<p><code>X.shape == (64, 20, 23)</code>을 보면 “배치 64개, 각 샘플은 과거 20시점, 시점당 23특성”이라고 읽는다. <code>Linear</code>는 마지막 축을 feature로 본다. <code>Conv1d</code>는 <code>[B,C,T]</code>, <code>RNN/LSTM/GRU</code>에 <code>batch_first=True</code>를 쓰면 <code>[B,T,F]</code>, <code>Conv2d</code>는 <code>[B,C,H,W]</code>다.</p>
<div class="code-shell"><div class="code-toolbar"><span>PYTHON</span><button type="button" data-copy-target="code-02-2">복사</button></div><pre id="code-02-2" tabindex="0" aria-label="2강 Python 코드 2"><code class="language-text">표형 MLP       [B, F]
시계열 원본    [B, T, F]
...
<p><code>reshape</code>는 원소 수를 바꾸지 않는다. <code>permute</code>는 축 순서를 바꾼다. 두 연산을 혼동하지 않는다.</p>
<h3 id="24-indexing">2.4 indexing과 경계</h3>
<p>Python slice의 끝은 포함되지 않는다.</p>
<div class="code-shell"><div class="code-toolbar"><span>PYTHON</span><button type="button" data-copy-target="code-02-3">복사</button></div><pre id="code-02-3" tabindex="0" aria-label="2강 Python 코드 3"><code class="language-python">import numpy as np
...
<p>Boolean mask의 길이는 대상 행 수와 같아야 한다. DataFrame에서는 label index와 positional index를 구분한다.</p>
<div class="code-shell"><div class="code-toolbar"><span>PYTHON</span><button type="button" data-copy-target="code-02-4">복사</button></div><pre id="code-02-4" tabindex="0" aria-label="2강 Python 코드 4"><code class="language-python"># label 기반: 인덱스 이름을 사용
row_a = df.loc["sample_a"]

# 위치 기반: 0번째 행을 사용
row_0 = df.iloc[0]
</code></pre></div>
<h3 id="25-copy-view">2.5 copy와 view</h3>
<p>원본을 보존하라는 함수에서 가장 안전한 시작은 <code>out = df.copy()</code>다. NumPy slicing은 원본을 가리키는 view일 수 있다.</p>
<div class="code-shell"><div class="code-toolbar"><span>PYTHON</span><button type="button" data-copy-target="code-02-5">복사</button></div><pre id="code-02-5" tabindex="0" aria-label="2강 Python 코드 5"><code class="language-python">a = np.arange(5)
b = a[1:4]       # view일 수 있음
b[0] = 999
assert a[1] == 999
...
<p>[Process 함정] 요구가 “입력 DataFrame을 변경하지 말고 반환”인데 입력 자체를 수정하면 보이는 예시는 통과해도 hidden test에서 실패할 수 있다.</p>
<h3 id="26-broadcasting">2.6 broadcasting</h3>
<p>뒤쪽 축부터 크기가 같거나 한쪽이 1이면 broadcasting이 가능하다.</p>
<ul>
<li><code>[B,F] + [F] → [B,F]</code></li>
<li><code>[B,T,F] + [F] → [B,T,F]</code></li>
<li><code>[B,T,F] + [T]</code>는 일반적으로 불가능하다. <code>[T]</code>는 마지막 축 <code>F</code>와 비교되기 때문이다.</li>
<li>시점별 값을 더하려면 <code>[1,T,1]</code>로 만든다.</li>
</ul>
<p>의도하지 않은 broadcasting은 오류 없이 잘못된 loss를 만든다. 예를 들어 예측 <code>[B,1]</code>과 정답 <code>[B]</code>를 MSE에 넣으면 <code>[B,B]</code>로 확장될 수 있다. 회귀에서는 둘 다 <code>[B]</code> 또는 둘 다 <code>[B,1]</code>로 맞춘다.</p>
<h3 id="27">2.7 함수 계약</h3>
<p>함수를 쓰기 전 주석으로 계약을 작성한다.</p>
<div class="code-shell"><div class="code-toolbar"><span>PYTHON</span><button type="button" data-copy-target="code-02-6">복사</button></div><pre id="code-02-6" tabindex="0" aria-label="2강 Python 코드 6"><code class="language-python">def selected_minmax(df, columns):
    """Contract
...
<p>실제 문제의 상수 열 처리·NaN 처리 지시가 다르면 그 지시를 따른다. “상식적으로 좋은 처리”보다 명세가 우선이다.</p>
<h3 id="28">2.8 실습문제</h3>
<ol>
<li><code>[32, 50, 8]</code> 센서 텐서를 <code>Conv1d(in_channels=8, ...)</code>에 넣기 직전 필요한 코드를 쓰라.</li>
<li>예측 <code>[128,1]</code>, 정답 <code>[128]</code>의 MSE에서 생길 수 있는 문제와 수정법을 쓰라.</li>
<li><code>a = np.arange(12).reshape(3,4)</code>에서 2번째 열만 <code>[3,1]</code>로 선택하는 코드를 두 가지 쓰라.</li>
<li>위 <code>selected_minmax</code>가 빈 <code>columns=[]</code>를 받으면 무엇을 반환해야 하는가?</li>
<li><code>reshape</code>와 <code>permute</code>의 차이를 한 문장으로 설명하라.</li>
</ol>
<details class="answer-panel" id="_3"><summary>정답·해설 보기</summary><div class="answer-body"><ol>
<li><code>x = x.permute(0, 2, 1)</code>로 <code>[B,T,F] → [B,F,T]</code>를 만든다.</li>
<li>broadcasting으로 <code>[128,128]</code> 비교가 될 수 있다. <code>pred.squeeze(1)</code> 또는 <code>target.unsqueeze(1)</code>로 정확히 같은 shape를 만든다.</li>
<li><code>a[:, 1:2]</code> 또는 <code>a[:, [1]]</code>다. <code>a[:,1]</code>은 <code>[3]</code>이 된다.</li>
<li>원본과 값이 같은 <strong>복사본</strong>을 반환하는 것이 계약에 자연스럽다. 입력 객체 자체를 반환하면 이후 mutation에 취약하다.</li>
<li>reshape는 원소를 유지하며 차원 크기를 다시 묶고, permute는 축의 순서 자체를 바꾼다.</li>
</ol></div></details>

<h3 id="29">2.9 완료 기준</h3>
<ul>
<li class="check-item"><label><input type="checkbox" data-check-id="02-1"><span>모든 모델 코드에 입력·출력 shape 주석을 붙인다.</span></label></li>
<li class="check-item"><label><input type="checkbox" data-check-id="02-2"><span>view와 copy 차이를 작은 배열로 재현했다.</span></label></li>
<li class="check-item"><label><input type="checkbox" data-check-id="02-3"><span><code>[B]</code>와 <code>[B,1]</code> loss broadcasting을 직접 확인했다.</span></label></li>
</ul>

10:["$","main",null,{"className":"lesson-main","id":"main-content","children":["$","article",null,{"children":[["$","header",null,{"className":"lesson-header","children":[["$","nav",null,{"className":"breadcrumbs","aria-label":"현재 위치","children":[["$","a",null,{"href":"/hdat-ds-study-hub/learn/","children":"34강"}],["$","span",null,{"children":"/"}],["$","span",null,{"children":[1,"단계"]}]]}],["$","p",null,{"className":"lesson-phase","children":[1,"단계 · ","시험과 기초 언어"]}],["$","h1",null,{"children":[["$","span",null,{"children":["02","강"]}],"Python·Jupyter·shape 언어"]}],["$","p",null,{"className":"lesson-summary","children":"축·인덱싱·복사·브로드캐스팅을 익혀 배열 코드의 입출력 shape를 읽는 습관을 만듭니다."}],["$","div",null,{"className":"lesson-meta","children":[["$","span",null,{"children":["약 ",50,"분"]}],["$","span",null,{"children":["코드 ",6,"개"]}],["$","span",null,{"children":["표 ",1,"개"]}],["$","span",null,{"children":["완료 체크 ",3,"개"]}]]}],["$","div",null,{"className":"tag-row","children":[["$","span","Python",{"children":"Python"}],["$","span","Shape",{"children":"Shape"}]]}],["$","p",null,{"className":"prerequisites","children":[["$","strong",null,{"children":"먼저 보면 좋은 강의"}],[["$","a","1",{"href":"/hdat-ds-study-hub/learn/01/","children":[1,"강"]}]]]}]]}],["$","details",null,{"className":"mobile-lesson-toc","children":[["$","summary",null,{"children":["이번 강의 목차 ",["$","span",null,{"children":[8,"개"]}]]}],["$","ol",null,{"children":[["$","li","22",{"children":["$","a",null,{"href":"#22","children":"시험 코드의 네 가지 객체"}]}],["$","li","23",{"children":["$","a",null,{"href":"#23","children":"축을 문장으로 읽기"}]}],["$","li","24-indexing",{"children":["$","a",null,{"href":"#24-indexing","children":"indexing과 경계"}]}],["$","li","25-copy-view",{"children":["$","a",null,{"href":"#25-copy-view","children":"copy와 view"}]}],["$","li","26-broadcasting",{"children":["$","a",null,{"href":"#26-broadcasting","children":"broadcasting"}]}],["$","li","27",{"children":["$","a",null,{"href":"#27","children":"함수 계약"}]}],["$","li","28",{"children":["$","a",null,{"href":"#28","children":"실습문제"}]}],["$","li","29",{"children":["$","a",null,{"href":"#29","children":"완료 기준"}]}]]}]]}],["$","section",null,{"className":"outcome-card","aria-labelledby":"outcome-title","children":[["$","p",null,{"className":"micro-label","children":"AFTER THIS LESSON"}],["$","h2",null,{"id":"outcome-title","children":"이번 강의가 끝나면"}],["$","ul",null,{"children":[["$","li","Python 함수의 입력 계약과 반환 계약을 명시한다.",{"children":[["$","span",null,{"aria-hidden":"true","children":"✓"}],"Python 함수의 입력 계약과 반환 계약을 명시한다."]}],["$","li","list, NumPy array, DataFrame, Tensor의 역할을 구분한다.",{"children":[["$","span",null,{"aria-hidden":"true","children":"✓"}],"list, NumPy array, DataFrame, Tensor의 역할을 구분한다."]}],["$","li","indexing, slicing, copy/view, broadcasting의 함정을 피한다.",{"children":[["$","span",null,{"aria-hidden":"true","children":"✓"}],"indexing, slicing, copy/view, broadcasting의 함정을 피한다."]}],["$","li","오류 메시지에서 최초의 shape 불일치를 찾는다.",{"children":[["$","span",null,{"aria-hidden":"true","children":"✓"}],"오류 메시지에서 최초의 shape 불일치를 찾는다."]}]]}]]}],["$","aside",null,{"className":"recall-note","children":[["$","strong",null,{"children":"처음 공부한다면"}],["$","p",null,{"children":"코드를 읽기 전에 입력과 출력 shape를 종이에 적고, 확인문제는 해설을 열기 전 30초 동안 먼저 설명해 보세요."}]]}],["$","section",null,{"aria-labelledby":"lesson-body-title","children":[["$","h2",null,{"id":"lesson-body-title","className":"sr-only","children":"강의 내용"}],["$","div",null,{"className":"lesson-content","dangerouslySetInnerHTML":{"__html":"$13"}}]]}],"$L14","$L15","$L16"]}]}]
11:["$","aside",null,{"className":"lesson-toc","aria-label":"이번 강의 목차","children":["$","div",null,{"children":[["$","p",null,{"className":"micro-label","children":"IN THIS LESSON"}],["$","ol",null,{"children":[["$","li","22",{"children":["$","a",null,{"href":"#22","children":"시험 코드의 네 가지 객체"}]}],["$","li","23",{"children":["$","a",null,{"href":"#23","children":"축을 문장으로 읽기"}]}],["$","li","24-indexing",{"children":["$","a",null,{"href":"#24-indexing","children":"indexing과 경계"}]}],["$","li","25-copy-view",{"children":["$","a",null,{"href":"#25-copy-view","children":"copy와 view"}]}],["$","li","26-broadcasting",{"children":["$","a",null,{"href":"#26-broadcasting","children":"broadcasting"}]}],["$","li","27",{"children":["$","a",null,{"href":"#27","children":"함수 계약"}]}],["$","li","28",{"children":["$","a",null,{"href":"#28","children":"실습문제"}]}],["$","li","29",{"children":["$","a",null,{"href":"#29","children":"완료 기준"}]}]]}]]}]}]
...
14:["$","section",null,{"className":"lesson-sources","aria-labelledby":"lesson-sources-title","children":[["$","div",null,{"className":"lesson-sources-heading","children":[["$","p",null,{"className":"micro-label","children":"PRIMARY SOURCES"}],["$","h2",null,{"id":"lesson-sources-title","children":"공식 자료로 한 번 더 확인하기"}],["$","p",null,{"children":"본문을 이해한 뒤 필요한 항목만 대조하세요. 외부 링크는 시험 전 학습용입니다."}]]}],["$","div",null,{"className":"source-list","children":[["$","a","https://numpy.org/doc/1.26/user/absolute_beginners.html",{"href":"https://numpy.org/doc/1.26/user/absolute_beginners.html","target":"_blank","rel":"noreferrer noopener","children":[["$","span",null,{"children":"NumPy·pandas 공식"}],["$","strong",null,{"children":"NumPy 1.26 absolute beginner guide"}],["$","p",null,{"children":"배열, 축, shape, indexing과 기본 연산을 작은 예제로 복습합니다."}],["$","small",null,{"children":"공식 페이지 열기 ↗"}]]}],["$","a","https://numpy.org/doc/1.26/user/basics.broadcasting.html",{"href":"https://numpy.org/doc/1.26/user/basics.broadcasting.html","target":"_blank","rel":"noreferrer noopener","children":[["$","span",null,{"children":"NumPy·pandas 공식"}],["$","strong",null,{"children":"NumPy broadcasting"}],["$","p",null,{"children":"shape가 다른 배열이 어떤 규칙으로 확장되는지 공식 규칙과 예제로 확인합니다."}],["$","small",null,{"children":"공식 페이지 열기 ↗"}]]}]]}]]}]
15:["$","section",null,{"className":"lesson-finish","aria-labelledby":"finish-title","children":[["$","div",null,{"children":[["$","p",null,{"className":"micro-label","children":"LESSON COMPLETE"}],["$","h2",null,{"id":"finish-title","children":"직접 설명하고 실행했다면 완료하세요"}],["$","p",null,{"children":"스크롤만 끝낸 것은 완료가 아닙니다. 이 강의의 완료 기준을 실제로 확인한 뒤 표시하세요."}]]}],["$","$L17",null,{"lessonId":"02"}]]}]
16:["$","nav",null,{"className":"lesson-pager","aria-label":"이전 및 다음 강의","children":[["$","a",null,{"href":"/hdat-ds-study-hub/learn/01/","children":[["$","span",null,{"children":"이전 강의"}],["$","strong",null,{"children":["← ",1,". ","시험 지도를 먼저 그리기"]}]]}],["$","a",null,{"className":"next","href":"/hdat-ds-study-hub/learn/03/","children":[["$","span",null,{"children":"다음 강의"}],["$","strong",null,{"children":[3,". ","NumPy·pandas로 데이터 계약 지키기"," →"]}]]}]]}]

...
e:["$","section","4",{"children":[["$","p",null,{"children":[["$","span",null,{"children":[4,"단계"]}],"실기 문제 완주"]}],["$","ol",null,{"children":[["$","li","26",{"data-progress-lesson":"26","children":["$","a",null,{"href":"/hdat-ds-study-hub/learn/26/","aria-current":"$undefined","children":[["$","span",null,{"children":"26"}],["$","strong",null,{"children":"Process형 완전 공략: 명세를 edge case까지 구현하기"}],["$","i",null,{"aria-hidden":"true","children":"✓"}]]}]}],["$","li","27",{"data-progress-lesson":"27","children":["$","a",null,{"href":"/hdat-ds-study-hub/learn/27/","aria-current":"page","children":[["$","span",null,{"children":"27"}],["$","strong",null,{"children":"Problem형 공통 파이프라인과 표형 문제"}],["$","i",null,{"aria-hidden":"true","children":"✓"}]]}]}],["$","li","28",{"data-progress-lesson":"28","children":["$","a",null,{"href":"/hdat-ds-study-hub/learn/28/","aria-current":"$undefined","children":[["$","span",null,{"children":"28"}],["$","strong",null,{"children":"시계열 Problem 완주"}],["$","i",null,{"aria-hidden":"true","children":"✓"}]]}]}],["$","li","29",{"data-progress-lesson":"29","children":["$","a",null,{"href":"/hdat-ds-study-hub/learn/29/","aria-current":"$undefined","children":[["$","span",null,{"children":"29"}],["$","strong",null,{"children":"이미지·이상탐지 Problem 분기"}],["$","i",null,{"aria-hidden":"true","children":"✓"}]]}]}],["$","li","30",{"data-progress-lesson":"30","children":["$","a",null,{"href":"/hdat-ds-study-hub/learn/30/","aria-current":"$undefined","children":[["$","span",null,{"children":"30"}],["$","strong",null,{"children":"제출·디버깅·170분 운영"}],["$","i",null,{"aria-hidden":"true","children":"✓"}]]}]}]]}]]}]
...
<h3 id="272">27.2 첫 셀: 문제 계약</h3>
<div class="code-shell"><div class="code-toolbar"><span>PYTHON</span><button type="button" data-copy-target="code-27-1">복사</button></div><pre id="code-27-1" tabindex="0" aria-label="27강 Python 코드 1"><code class="language-python">TASK = "regression"       # regression/binary/multiclass/multilabel
TARGET_COLS = ["target"]
ID_COLS = ["id"]
DROP_COLS = []
METRIC = "rmse"
...
<ul>
<li>제출은 label인가 probability인가 raw regression인가?</li>
<li>output shape는 <code>(N,)</code>, <code>(N,1)</code>, <code>(N,K)</code> 중 무엇인가?</li>
<li>class label 원형과 column 순서는?</li>
<li>파일명·dtype·저장 셀은?</li>
</ul>
<h3 id="273-10-eda-split">27.3 10분 EDA와 split 결정</h3>
<ol>
<li>shape/schema/target 결측</li>
<li>row 의미와 group/time 존재</li>
<li>class/target distribution</li>
<li>누수 후보와 ID</li>
<li>official metric</li>
</ol>
<p>그 뒤 split을 고정한다. 모델마다 split을 바꾸면 비교할 수 없다.</p>
<h3 id="274-baseline">27.4 baseline의 역할</h3>
<p>baseline은 세 가지를 검증한다.</p>
<ul>
<li>전처리와 데이터 계약이 실행되는가?</li>
<li>validation metric 계산이 맞는가?</li>
<li>제출 파일이 생성되는가?</li>
</ul>
<p>표형에서는 선형/ExtraTrees 같은 빠른 baseline이 PyTorch MLP보다 강할 수 있다. 사용자 요구대로 딥러닝 학습은 PyTorch만 사용하지만, 고전 baseline은 scikit-learn으로 빠르게 확보할 수 있다.</p>
<h3 id="275-pytorch-dense-bridge">27.5 PyTorch dense bridge</h3>
<p>희소 matrix를 MLP 입력으로 바꿀 때 메모리를 계산한다.</p>
<div class="code-shell"><div class="code-toolbar"><span>PYTHON</span><button type="button" data-copy-target="code-27-2">복사</button></div><pre id="code-27-2" tabindex="0" aria-label="27강 Python 코드 2"><code class="language-python">def to_float32_dense(x, max_gib=2.0):
...
<p>one-hot이 너무 크면 high-cardinality category 처리 방식을 바꾼다.</p>
<h3 id="276-task-label">27.6 task별 label</h3>
<p>Binary original label이 <code>['normal','fault']</code>면 positive class를 명시한다.</p>
<div class="code-shell"><div class="code-toolbar"><span>PYTHON</span><button type="button" data-copy-target="code-27-3">복사</button></div><pre id="code-27-3" tabindex="0" aria-label="27강 Python 코드 3"><code class="language-python">POSITIVE_LABEL = "fault"
y_bin = (train[TARGET_COLS[0]].to_numpy() == POSITIVE_LABEL).astype(np.float32)
</code></pre></div>
<p>Multiclass:</p>
<div class="code-shell"><div class="code-toolbar"><span>PYTHON</span><button type="button" data-copy-target="code-27-4">복사</button></div><pre id="code-27-4" tabindex="0" aria-label="27강 Python 코드 4"><code class="language-python">classes = np.sort(train[TARGET_COLS[0]].unique())
class_to_idx = {c: i for i, c in enumerate(classes)}
idx_to_class = np.asarray(classes)
y_idx = train[TARGET_COLS[0]].map(class_to_idx).to_numpy(dtype=np.int64)
</code></pre></div>
<p>Prediction은 <code>idx_to_class[pred_idx]</code>로 복원한다. probability 열 순서는 <code>classes</code> 순서다.</p>
<h3 id="277-mlp">27.7 MLP 선택</h3>
<ul>
<li>회귀 output D, MSE/Huber</li>
<li>binary output 1, BCEWithLogits</li>
<li>multiclass output K, CE</li>
<li>multilabel output K, BCEWithLogits</li>
</ul>
<p>hidden <code>(128,64)</code>부터 시작하고 dropout 0~0.2. feature 수·데이터 크기에 맞게 축소한다. training time cap을 둔다.</p>
<h3 id="278-validation">27.8 validation→전체 재학습</h3>
<p>Validation으로 preprocessing·모델·epoch·threshold를 정한다. 최종에는:</p>
<ol>
<li>train 전체에 preprocessing 새로 fit</li>
<li>validation에서 고른 epoch 수/설정으로 model 새로 학습</li>
<li>test transform·predict</li>
<li>inverse label/target transform</li>
<li>submission 검증</li>
</ol>
<p>전체 재학습에서 validation early stopping이 없으므로 best epoch를 사용하거나 작은 holdout을 남길 수 있다. 시간과 점수 안정성을 고려한다.</p>
<h3 id="279">27.9 오류 분석</h3>
<p>회귀:</p>
<ul>
<li>큰 absolute error 행</li>
<li>target 구간/group별 RMSE</li>
<li>residual mean·분산</li>
</ul>
<p>분류:</p>
<ul>
<li>confusion matrix</li>
<li>class별 recall/F1</li>
<li>확신 높은 오답</li>
<li>group/time/category별 성능</li>
</ul>
<p>오류 분석 후 한 번에 한 변경만 한다.</p>
<h3 id="2710">27.10 원본 연습: 차량 효율 회귀</h3>
<p>다음 synthetic 문제를 직접 만든다.</p>
<div class="code-shell"><div class="code-toolbar"><span>PYTHON</span><button type="button" data-copy-target="code-27-5">복사</button></div><pre id="code-27-5" tabindex="0" aria-label="27강 Python 코드 5"><code class="language-python">rng = np.random.default_rng(42)
n = 6000
...
<p>과제:</p>
<ol>
<li>80/20 split과 RMSE baseline</li>
<li>median+one-hot+scale</li>
<li>PyTorch MLP</li>
<li>group 열을 새로 합성해 group split과 random split 비교</li>
<li>5% 결측을 넣고 안전하게 처리</li>
<li>prediction <code>(N,1)</code>과 <code>(N,)</code> 제출 계약 비교</li>
</ol>
<p>불균형 이진분류를 데이터 생성부터 group split, PyTorch MLP, threshold, 전체 재학습, NPY reload까지 실행하는 모범 실습은 같은 폴더의 <code>full_mock_tabular.py</code>에 있다. 먼저 <code>--generate-only</code>로 train/test만 만든 뒤 혼자 풀고, 그 다음 기본 실행 결과와 비교한다.</p>
<h3 id="2711">27.11 실습문제</h3>
<ol>
<li>MLP가 tree baseline보다 낮으면 반드시 코딩 오류인가?</li>
<li>multiclass probability column 순서를 어디서 얻는가?</li>
<li>validation에서 target scaler를 inverse하지 않고 RMSE를 계산하면 무엇을 측정하는가?</li>
<li>전체 재학습 때 validation threshold를 test에서 다시 고쳐도 되는가?</li>
<li>첫 baseline을 빨리 저장해야 하는 이유 세 가지는?</li>
</ol>
<details class="answer-panel" id="_28"><summary>정답·해설 보기</summary><div class="answer-body"><ol>
<li>아니다. 표형 데이터에서 tree ensemble이 더 강할 수 있다. shape/loss를 확인한 뒤 validation 결과를 따른다.</li>
<li>학습 때 고정한 label encoder/classes 순서.</li>
<li>표준화된 target 공간의 RMSE로 공식 단위와 다르다.</li>
<li>안 된다. test 누수다.</li>
<li>파이프라인·metric·제출 계약 검증, 시간 부족 보험, 이후 개선의 비교 기준.</li>
</ol></div></details>

<h3 id="2712">27.12 완료 기준</h3>
<ul>
<li class="check-item"><label><input type="checkbox" data-check-id="27-1"><span>네 task의 출력을 10분 안에 구성한다.</span></label></li>
<li class="check-item"><label><input type="checkbox" data-check-id="27-2"><span>baseline→MLP→전체 재학습→제출을 한 번 완주한다.</span></label></li>
<li class="check-item"><label><input type="checkbox" data-check-id="27-3"><span>원 label·target 단위로 metric과 제출을 복원한다.</span></label></li>
</ul>

10:["$","main",null,{"className":"lesson-main","id":"main-content","children":["$","article",null,{"children":[["$","header",null,{"className":"lesson-header","children":[["$","nav",null,{"className":"breadcrumbs","aria-label":"현재 위치","children":[["$","a",null,{"href":"/hdat-ds-study-hub/learn/","children":"34강"}],["$","span",null,{"children":"/"}],["$","span",null,{"children":[4,"단계"]}]]}],["$","p",null,{"className":"lesson-phase","children":[4,"단계 · ","실기 문제 완주"]}],["$","h1",null,{"children":[["$","span",null,{"children":["27","강"]}],"Problem형 공통 파이프라인과 표형 문제"]}],["$","p",null,{"className":"lesson-summary","children":"문제 계약과 baseline에서 시작해 표형 데이터의 학습·검증·재학습·오류 분석을 완주합니다."}],["$","div",null,{"className":"lesson-meta","children":[["$","span",null,{"children":["약 ",105,"분"]}],["$","span",null,{"children":["코드 ",5,"개"]}],["$","span",null,{"children":["표 ",0,"개"]}],["$","span",null,{"children":["완료 체크 ",3,"개"]}]]}],["$","div",null,{"className":"tag-row","children":[["$","span","Problem형",{"children":"Problem형"}],["$","span","표형데이터",{"children":"표형데이터"}],["$","span","PyTorch",{"children":"PyTorch"}]]}],["$","p",null,{"className":"prerequisites","children":[["$","strong",null,{"children":"먼저 보면 좋은 강의"}],[["$","a","8",{"href":"/hdat-ds-study-hub/learn/08/","children":[8,"강"]}],["$","a","13",{"href":"/hdat-ds-study-hub/learn/13/","children":[13,"강"]}],["$","a","14",{"href":"/hdat-ds-study-hub/learn/14/","children":[14,"강"]}],["$","a","16",{"href":"/hdat-ds-study-hub/learn/16/","children":[16,"강"]}],["$","a","17",{"href":"/hdat-ds-study-hub/learn/17/","children":[17,"강"]}],["$","a","18",{"href":"/hdat-ds-study-hub/learn/18/","children":[18,"강"]}]]]}]]}],["$","details",null,{"className":"mobile-lesson-toc","children":[["$","summary",null,{"children":["이번 강의 목차 ",["$","span",null,{"children":[11,"개"]}]]}],["$","ol",null,{"children":[["$","li","272",{"children":["$","a",null,{"href":"#272","children":"첫 셀: 문제 계약"}]}],["$","li","273-10-eda-split",{"children":["$","a",null,{"href":"#273-10-eda-split","children":"10분 EDA와 split 결정"}]}],["$","li","274-baseline",{"children":["$","a",null,{"href":"#274-baseline","children":"baseline의 역할"}]}],["$","li","275-pytorch-dense-bridge",{"children":["$","a",null,{"href":"#275-pytorch-dense-bridge","children":"PyTorch dense bridge"}]}],["$","li","276-task-label",{"children":["$","a",null,{"href":"#276-task-label","children":"task별 label"}]}],["$","li","277-mlp",{"children":["$","a",null,{"href":"#277-mlp","children":"MLP 선택"}]}],["$","li","278-validation",{"children":["$","a",null,{"href":"#278-validation","children":"validation→전체 재학습"}]}],["$","li","279",{"children":["$","a",null,{"href":"#279","children":"오류 분석"}]}],["$","li","2710",{"children":["$","a",null,{"href":"#2710","children":"원본 연습: 차량 효율 회귀"}]}],["$","li","2711",{"children":["$","a",null,{"href":"#2711","children":"실습문제"}]}],["$","li","2712",{"children":["$","a",null,{"href":"#2712","children":"완료 기준"}]}]]}]]}],["$","section",null,{"className":"outcome-card","aria-labelledby":"outcome-title","children":[["$","p",null,{"className":"micro-label","children":"AFTER THIS LESSON"}],["$","h2",null,{"id":"outcome-title","children":"이번 강의가 끝나면"}],["$","ul",null,{"children":[["$","li","10분 안에 task·metric·split·제출 계약을 확정한다.",{"children":[["$","span",null,{"aria-hidden":"true","children":"✓"}],"10분 안에 task·metric·split·제출 계약을 확정한다."]}],["$","li","표형 baseline으로 첫 유효 제출 파일을 만든다.",{"children":[["$","span",null,{"aria-hidden":"true","children":"✓"}],"표형 baseline으로 첫 유효 제출 파일을 만든다."]}],["$","li","PyTorch MLP를 같은 validation에서 비교한다.",{"children":[["$","span",null,{"aria-hidden":"true","children":"✓"}],"PyTorch MLP를 같은 validation에서 비교한다."]}],["$","li","회귀·이진·다중분류·multilabel·다중출력을 분기한다.",{"children":[["$","span",null,{"aria-hidden":"true","children":"✓"}],"회귀·이진·다중분류·multilabel·다중출력을 분기한다."]}]]}]]}],["$","aside",null,{"className":"recall-note","children":[["$","strong",null,{"children":"처음 공부한다면"}],["$","p",null,{"children":"코드를 읽기 전에 입력과 출력 shape를 종이에 적고, 확인문제는 해설을 열기 전 30초 동안 먼저 설명해 보세요."}]]}],["$","section",null,{"aria-labelledby":"lesson-body-title","children":[["$","h2",null,{"id":"lesson-body-title","className":"sr-only","children":"강의 내용"}],["$","div",null,{"className":"lesson-content","dangerouslySetInnerHTML":{"__html":"$13"}}]]}],"$L14","$L15","$L16"]}]}]
11:["$","aside",null,{"className":"lesson-toc","aria-label":"이번 강의 목차","children":["$","div",null,{"children":[["$","p",null,{"className":"micro-label","children":"IN THIS LESSON"}],["$","ol",null,{"children":[["$","li","272",{"children":["$","a",null,{"href":"#272","children":"첫 셀: 문제 계약"}]}],["$","li","273-10-eda-split",{"children":["$","a",null,{"href":"#273-10-eda-split","children":"10분 EDA와 split 결정"}]}],["$","li","274-baseline",{"children":["$","a",null,{"href":"#274-baseline","children":"baseline의 역할"}]}],["$","li","275-pytorch-dense-bridge",{"children":["$","a",null,{"href":"#275-pytorch-dense-bridge","children":"PyTorch dense bridge"}]}],["$","li","276-task-label",{"children":["$","a",null,{"href":"#276-task-label","children":"task별 label"}]}],["$","li","277-mlp",{"children":["$","a",null,{"href":"#277-mlp","children":"MLP 선택"}]}],["$","li","278-validation",{"children":["$","a",null,{"href":"#278-validation","children":"validation→전체 재학습"}]}],["$","li","279",{"children":["$","a",null,{"href":"#279","children":"오류 분석"}]}],["$","li","2710",{"children":["$","a",null,{"href":"#2710","children":"원본 연습: 차량 효율 회귀"}]}],["$","li","2711",{"children":["$","a",null,{"href":"#2711","children":"실습문제"}]}],["$","li","2712",{"children":["$","a",null,{"href":"#2712","children":"완료 기준"}]}]]}]]}]}]

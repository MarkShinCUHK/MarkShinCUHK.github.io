...
한 차량의 사진만 보면 외관은 알 수 있지만 센서의 온도까지 알 수는 없다. 사진과 센서를 함께 사용하려면 모양이 다른 두 입력을 먼저 각자 처리한 뒤 합쳐야 한다. 여기서 <strong>인코더(encoder)</strong>는 입력을 모델이 사용하기 좋은 숫자 묶음으로 바꾸는 부분이다. **특징 벡터**는 그렇게 얻은 숫자 한 줄이고, <strong>출력 헤드(head)</strong>는 그 숫자들로 원하는 종류의 정답을 계산하는 마지막 부분이다.

아래 모델의 한 사례를 먼저 따라가 보자. 사진에서 특징 8개, 센서에서 특징 6개를 만들고 같은 사례끼리 이어 붙이면 14개다. 이것을 공통 특징 12개로 바꾼 뒤 한 갈래는 클래스 3개의 점수, 다른 갈래는 연속 측정값 2개를 만든다. B개를 한 번에 처리하면 [B,8]과 [B,6]을 **열 방향**으로 합쳐 [B,14]가 된다. 행 방향으로 합치는 것은 입력 종류를 합치는 일이 아니라 사례 수를 늘리는 일이므로 다르다.

> **먼저 확인** · 두 입력의 행 수가 같다는 것만으로 충분하지 않다. 첫 사진과 첫 센서가 실제로 같은 차량·같은 예측 시점의 기록인지도 같아야 한다. 이어지는 shape 검사와 ID 대응 검사는 서로 다른 문제를 잡는다.
...
입력도 여러 시점이고 정답도 여러 시점이라면, 하나의 고정 숫자를 예측하는 것만으로는 부족하다. 예를 들어 관측 구간을 읽고 다음 세 값을 순서대로 만들거나, 문장을 읽고 다른 문장을 단어별로 만들 수 있다. **Seq2Seq**는 sequence-to-sequence의 줄임말로 입력 수열에서 출력 수열을 만드는 구조다. 입력을 읽는 인코더와 출력을 한 단계씩 만드는 디코더를 구분한다.

아래에서는 종류 번호로 된 작은 토큰 수열을 쓴다. 토큰은 수열의 한 항목이고, BOS는 시작 표시, EOS는 끝 표시, PAD는 배치의 길이를 맞추기 위한 빈자리 표시다. 실제 단어를 만드는 성능 실험이 아니라 입력 위치와 정답 위치를 맞추는 연습이다. 한 단계에서 정답 자체를 넣으면 맞히는 대신 읽어 버릴 수 있으므로, 무엇을 이미 알려 준 상태에서 다음 항목을 맞히는지 확인해야 한다.

| 단계 | 학습 때 디코더에 주는 것 | 이번 단계의 정답 | 추론 때 디코더에 주는 것 |
|---|---|---|---|
| 1 | BOS | a | BOS |
| 2 | 이전 정답 a | b | 1단계의 예측 |
| 3 | 이전 정답 b | EOS | 2단계의 예측 |

이 표의 학습 방식이 티처 포싱이다. 추론에는 정답 a·b가 없으므로 자신의 직전 예측으로 이어 간다. 그래서 학습에서 틀릴 기회가 적었던 입력과 실제 추론 입력이 달라질 수 있다. 그 차이까지 떠올린 뒤 다음 코드를 읽자.
...
9강에서 새 축으로 좌표를 줄였다면, 이번에는 그 좌표로 원래 값이 얼마나 복원되는지 확인한다. [1,1], [2,2], [3,3]처럼 한 직선에 있는 점은 그 직선에서의 위치 하나로 복원할 수 있다. 직선 밖의 흔들림까지 있는 점에서 축 하나를 버리면 그 흔들림은 복원되지 않는다. <strong>역변환(inverse transform)</strong>은 남긴 좌표를 원래 공간으로 되돌리는 계산이지 버린 정보를 되찾는 마법이 아니다.

SVD는 원래 행렬을 세 행렬의 곱으로 나누어 표현하는 특이값 분해다. 여기서는 복잡한 행렬의 계산을 외우기보다 어느 행렬이 ‘새 방향’, 어느 숫자가 ‘그 방향의 크기’를 나타내는지 따라간다. N은 사례 수, F는 입력 특성 수, k는 남길 축 수다. 평균을 뺀 Xc에서 축을 골라 Z를 만든 뒤 같은 축을 이용해 복원하고 학습 평균을 다시 더한다. 아래 모양 표기는 그 단계들에서 행렬곱이 가능한지 확인하는 용도다.
...
모델을 한 번 학습하는 데 30분이 걸린다고 하자. 학습률·층 수·배치 크기의 모든 조합을 시험할 시간이 없으면 다음에 어떤 조합을 시도할지 잘 골라야 한다. **하이퍼파라미터**는 이런 학습 전에 정하는 설정이고, 학습으로 갱신되는 가중치와 다르다. 베이즈 탐색은 이미 해 본 실험을 이용해 다음 실험 후보를 고르는 방법이다.

현재 가장 좋은 점수가 0.80이고 후보 A의 예상 점수가 0.81, 후보 B는 0.78이라고 하자. 평균만 보면 A가 좋아 보인다. 그러나 B를 거의 시험하지 않아 결과의 불확실성이 크다면 실제로 0.80을 넘을 가능성도 있다. ‘현재 좋아 보이는 곳’과 ‘아직 잘 모르는 곳’을 함께 살펴보려는 것이 탐색의 동기다. <strong>기대 개선량(EI)</strong>은 가능한 결과 각각의 개선량을 그 가능성으로 평균한 값이지 실제로 확인한 개선량이 아니다.

뒤의 식에서 μ는 후보 점수의 예측 평균, σ는 불확실성을 나타내는 표준편차, ξ는 개선을 판단할 여유 값이다. Φ와 φ는 각각 표준정규분포의 누적확률과 확률밀도 함수다. 점수는 클수록 좋은지, 오차는 작을수록 좋은지부터 정하고 식을 읽어야 부호를 뒤집지 않는다. 추천받은 후보도 반드시 실제 학습·검증을 해야 결과를 알 수 있다.

...
필기는 평균 2분 30초에 한 문제다. 긴 유도보다 **정의, 방향, 출력 범위, shape, 장단점**을 빠르게 판별해야 한다. Process는 코드가 짧아도 함수명, 반환형, 원본 보존, 경계조건을 틀리면 hidden test를 잃는다. Problem은 가장 화려한 모델보다 **유효한 제출 파일을 먼저 확보하는 능력**이 중요하다.
...
실기는 개인 로컬 자료와 허용된 단방향 검색을 참고할 수 있지만, 생성형 AI·메신저·메일·협업 서비스 등 외부와 답을 주고받는 수단은 금지된다. 데이터는 제공 IDE 밖으로 내려받아 처리하지 않는다. 공식 지침과 감독관 안내가 최우선이다.
...
| 125~150분 | 남은 Process와 hidden edge case | 8문항 모두 입력 |
...
1. 자동 hidden test가 함수명·반환형·shape·경계조건을 검사할 수 있고, 요구한 동작과 다른 “더 좋은” 구현도 오답이 될 수 있기 때문이다.
2. 최소한 실행 가능한 baseline과 요구된 이름·shape의 예측 파일이다.
3. 시험 전에 충분히 학습하고, 시험 중에는 허용된 로컬 오픈북으로 짧은 공식·코드 조각을 검색한다. 당시 규정이 허용하는 범위만 따른다.
...
## 26. Process형 완전 공략: 명세를 hidden test까지 구현하기
...
- 시험장에서 찾는 짧은 코드: `../openbook/HDAT_DS_실기_오픈북_치트시트.html`
- 재사용 source: `../openbook/hdat_templates.py`
- 조사 근거와 자료 오류: `../HDAT_DS_2026_조사_분석.md`
- 기존 일정표: `../HDAT_DS_2026_23일_학습계획.md`

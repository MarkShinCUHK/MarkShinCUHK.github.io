...
먼저 ‘코드를 저장했다’와 ‘코드를 실행했다’를 구별하자. 계산식 `2+3`이 적힌 파일을 저장하는 것은 설명서를 보관하는 일이고, 실행은 Python에게 그 계산을 하게 하는 일이다. Jupyter는 코드와 실행 결과를 한 문서에 놓지만, 문서에 보이는 순서와 실제로 실행한 순서가 다를 수 있다. 이번 강의에서는 설치 명령을 외우기보다 **어디에 입력하고, 무엇을 실행하며, 결과를 어디서 확인하는가**를 먼저 익힌다.
...
작은 상황부터 생각해 보자. 센서가 문자로 보내 준 `"10"`과 `"20"`을 그대로 더하면 `"1020"`이라는 연결된 글자가 된다. 숫자 10과 20으로 바꾼 뒤 더해야 30이 된다. 연산자가 같아도 **자료형(type), 즉 값의 종류에 따라 연산의 의미가 다르다**. 그래서 출력이 이상할 때 계산식만 보지 않고 그 값이 숫자인지 문자열인지도 확인한다.

또 `count=3` 뒤에 `count=count+1`을 실행하면 오른쪽에서 기존 3에 1을 더한 다음 왼쪽 이름을 새 값 4에 붙인다. 수학에서 3=4라는 주장을 하는 것이 아니다. 코드의 각 줄을 ‘오른쪽 계산 → 결과에 이름 붙이기’ 순서로 읽으면 재대입도 자연스럽게 이해할 수 있다.
...
여기서 샘플은 학생 한 명이나 차량 한 대처럼 관측한 사례 하나다. 세 학생 점수 [80,95,72]에서 ‘둘째 값’을 찾으려면 위치가 필요하고, ‘민수의 점수’를 찾으려면 이름이 필요하다. 위치로 찾을 값 묶음에는 리스트, 이름표로 찾을 대응에는 딕셔너리가 잘 맞는다. 같은 자료를 어떤 질문에 답하기 쉽게 저장할지 정하는 것이 자료구조를 고르는 이유다.

인덱싱(indexing)은 그중 필요한 값을 꺼내는 작업이다. 인덱스 0은 첫 값, 1은 두 번째 값이므로 사람의 ‘몇 번째’와 코드의 번호가 하나 다르다. 이어지는 예제를 실행하기 전에 찾으려는 값을 손가락으로 먼저 짚어 보자. 번호를 외우기보다 번호가 어느 값을 가리키는지 확인하는 습관이 이후 입력과 정답을 맞추는 데에도 필요하다.
...
온도 세 개 [18,27,35]에서 30 이상인 값의 개수를 세어 보자. 사람이 하면 18은 제외, 27도 제외, 35는 포함해서 답은 1이다. 이를 코드로 옮기려면 ‘값을 하나씩 본다’는 **반복**, ‘30 이상인가’라는 **조건**, ‘맞으면 개수를 1 늘린다’는 **대입**이 필요하다. 함수는 이 절차에 이름을 붙여 다른 온도 목록에도 재사용할 수 있게 만든다.

함수가 화면에 1을 보여 주는 것과 호출한 곳에 1을 돌려주는 것은 다르다. `print`는 확인용 출력이고 `return`은 다음 계산이 사용할 결과를 전달한다. 뒤의 예제에서 반환값을 다시 평균이나 비교에 쓰는 이유를 함께 확인하자. 오류가 나면 무조건 숨기기보다 어느 입력에서 어떤 조건이 깨졌는지 찾는 것이 먼저다.
...
온도 [10,20,30]을 각각 두 배로 바꿔 [20,40,60]을 만들고 싶다고 하자. Python 리스트에 `*2`를 하면 목록 자체가 두 번 이어진다. NumPy 배열에 `*2`를 하면 각 숫자가 두 배가 된다. **벡터화**란 이런 여러 원소의 계산을 배열 연산으로 표현하는 것이다. 값 묶음을 반복하는 것과 원소마다 계산하는 것의 차이를 먼저 이해한 뒤 코드를 실행해 보자.

shape는 ‘값이 몇 축에 어떻게 놓였는가’, dtype은 ‘각 값을 어떤 숫자 형식으로 저장했는가’다. 온도 세 개를 한 줄에 놓은 (3,)과 차량 세 대의 온도·속도를 놓은 (3,2)는 원소 수뿐 아니라 의미가 다르다. 이번 강의에서는 숫자 결과와 함께 이 두 정보를 읽는다.
...
가장 중요한 질문은 ‘어느 값끼리 계산하려는가?’다. 두 학생의 정답 [1,2]와 예측 [1,2]를 같은 학생끼리 빼면 차이는 [0,0]이다. 하지만 정답을 한 줄, 예측을 세로 두 줄로 두면 배열 규칙에 따라 모든 쌍의 차이 [[0,−1],[1,0]]를 만들 수 있다. 오류 없이 계산됐어도 원래 풀려던 문제가 달라진 것이다. 여기서 B는 한 번에 처리할 사례 수이며, 숫자 값 자체가 아니다.

축(axis)은 배열에서 위치를 지정하는 방향이다. 브로드캐스팅은 크기가 1인 축 등의 규칙을 이용해 서로 다른 모양의 배열을 함께 계산하는 방식이다. 편리한 자동 처리인 만큼, 먼저 원하는 대응을 말로 적고 그다음 shape가 그 대응을 나타내는지 확인한다. 이 장에서 단순한 표부터 여러 축의 텐서까지 같은 질문으로 확장한다.
...
온도 열 하나를 고른다고 해도 ‘이름표가 붙은 값 한 줄’과 ‘열이 하나인 표’는 구분할 필요가 있다. `df["temp"]`는 Series로, `df[["temp"]]`는 DataFrame으로 돌려준다. 사례가 세 개라면 모양은 각각 (3,)과 (3,1)이다. 두 번째 대괄호는 열 이름 하나를 담은 목록을 뜻한다. 숫자는 같아 보여도 모델이 ‘사례×특성’ 형태의 표를 요구하면 두 번째 형태가 필요하다.

행을 고를 때도 ‘인덱스 이름 10인 행’과 ‘열한 번째 위치의 행’은 다를 수 있다. 이 차이가 이름으로 찾는 `loc`와 위치로 찾는 `iloc`를 구별하는 이유다. 데이터를 정렬하거나 일부 행을 지운 뒤에는 인덱스 이름이 화면의 줄 번호와 같다고 가정하지 않는다.
...
처리 전후에 ‘한 행이 무엇인가’를 놓치지 말자. 차량 A의 운행 두 건 [10,20]을 그룹 평균으로 요약하면 A의 평균 15라는 한 행이 된다. 그룹별 집계는 단순한 정렬이 아니라 **관측 단위가 달라지는 작업**이다. 반대로 운행 두 행에 차량 A의 정보 한 행을 연결하면 보통 두 운행 행을 유지하고 설명 열만 늘어나야 한다. 연결할 정보표에 A가 두 번 있으면 예상보다 행이 늘 수 있어 병합 조건을 검사한다.

데이터 누수는 평가용 자료나 실제 예측 순간에 모를 정보가 규칙을 정하는 데 미리 들어오는 문제다. 여기서는 원리를 처음 만나므로 ‘빈칸을 채울 중앙값을 어느 행들로 계산했는가’부터 추적해 보자. 학습 행에서 중앙값 15를 구했다면 평가 행도 그 15로 채운다. 평가 행에서 별도로 중앙값을 구하는 것이 아니다. 본과정 8강에서 그 이유를 같은 자로 잰다는 예와 함께 다시 다룬다.
...
속도 [60,60,60]과 [0,60,120]은 둘 다 평균 60이다. 평균만 보고 두 운행이 비슷하다고 결론 내리면 두 번째 운행의 큰 변화를 놓친다. 시간에 따라 어떻게 달라졌는지는 꺾은선으로, 값이 어디에 많이 모였는지는 히스토그램으로 본다. 같은 데이터를 그려도 **무슨 질문에 답할지에 따라 그래프가 달라진다**.

코드에서는 Figure가 그림 전체이고 Axes가 축과 데이터를 그릴 개별 영역이다. 종이 하나에 그래프 하나를 그릴 수도 있고 네 개를 나눠 그릴 수도 있다고 생각하면 된다. 이어지는 코드에서 `fig`는 전체 그림 저장에, `ax`는 선·점·축 이름을 지정하는 데 쓰이는 이유가 여기에 있다. 그래프를 그린 뒤에는 단위와 시간 순서가 의도와 같은지도 확인한다.
...
이번에는 도구의 이름보다 하나의 행을 끝까지 따라가 보자. 차량 A의 온도·속도는 입력 X의 한 행이고, A의 고장 여부는 정답 y의 대응 값이다. pandas에서 표를 정리한 뒤 NumPy 배열로 바꾸고, 다시 PyTorch 텐서로 바꾸더라도 **A의 입력과 A의 정답이 계속 같은 위치에 있어야** 한다. 표를 숫자 배열로 바꾸면 열 이름이 자동으로 따라가지 않으므로 열 순서도 기록한다.

학습 데이터는 변환 기준을 배울 행, 검증 데이터는 그 기준을 새 사례에 적용해 확인할 행이다. 먼저 나누고 학습 부분에서만 중앙값·평균·표준편차를 구한다. 그 기준으로 양쪽을 변환한 다음 한 번에 처리할 작은 묶음인 배치를 만든다. 아직 신경망 학습을 몰라도 여기까지 각 값의 출처와 모양을 설명하면 이번 입문 과정의 핵심을 익힌 것이다.

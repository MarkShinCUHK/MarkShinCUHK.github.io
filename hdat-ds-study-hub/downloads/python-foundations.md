# HDAT-DS Python 기초 10강 — Python이라는 이름만 아는 사람부터
...
# 입문 1강. 실행 환경, 파일, Jupyter Notebook

## 이 과정은 어떻게 공부하나요?

처음부터 코드를 외우려고 하지 않아도 된다. 코드가 하는 일을 한 줄씩 말로 설명할 수 있게 되는 것이 먼저다. 다음 순서를 매 강 반복한다.
...
권장 시간은 강의당 90–150분이다. 한 번에 이해되지 않으면 정상이다. 특히 5–10강은 하루에 한 강씩 공부하고 다음 날 같은 코드를 빈 화면에서 다시 써 본다.

### 코드를 어디에서 실행하나요?

가장 쉬운 환경은 Jupyter Notebook이다. 한 칸을 <strong>셀(cell)</strong>이라고 부르며, 셀 안에 코드를 넣고 `Shift + Enter`를 누르면 실행된다. `.py` 파일로 공부해도 된다. 이 교재의 Python 코드 블록은 필요한 import와 예제 데이터를 각각 포함하므로, 원칙적으로 <strong>새 노트북의 빈 셀 하나에 코드 블록 전체를 붙여 넣어 독립 실행</strong>할 수 있다.
...
|---|---|---|---|
| Python `list` | 값을 순서대로 담는 일반 상자 | `[10, 20, 30]` | `len(x)`, `type(x)` |
...
코드가 틀리지 않았는데도 "파일을 찾을 수 없습니다", "모듈이 없습니다", "아까 만든 변수가 없습니다" 같은 오류가 날 수 있다. 대개 **어떤 Python을 실행 중인지**, **현재 어느 폴더를 보고 있는지**, **노트북 셀을 어떤 순서로 실행했는지**가 원인이다. 데이터 분석을 시작하기 전에 실행 환경을 읽는 법부터 알아야 이후 오류를 스스로 해결할 수 있다.
...
**선택 미리보기 — 오늘 외우거나 재현할 필요는 없다.** 아래 파일 쓰기와 `with` 문법은 함수·들여쓰기 학습 후 돌아와도 된다. 1강에서는 실행 위치와 출력만 확인하면 충분하다. 외부 파일 없이 임시 폴더를 사용하며, `with` 블록이 끝나면 임시 파일은 자동 정리된다.
...
다음 문제는 변수·튜플·조건문을 먼저 배운 사람을 위한 확장이다. 1강 완료 조건이 아니다. 정답을 열기 전에 새 셀에서 직접 해결한다.
...
# 입문 3강. list, tuple, dict, set와 index·slice
...
예상 출력은 차례로 `5`, `25`, `[10, 15, 20]`이다. index 4는 slice에 포함되지 않는다.
...
예상 출력: `32 10 5`. 이것을 tuple unpacking이라고 한다.
...
- `assert`로 코드가 기대 조건을 만족하는지 확인한다.
...
index와 값을 함께 원하면 `enumerate`를 쓴다.
...
함수 이름은 동사로, 입력과 반환을 분명하게 만든다. `print`와 `return`은 다르다. `print`는 화면에 보여 주지만 호출 결과는 기본적으로 `None`이다. `return`은 값을 바깥으로 보낸다.
...
예외는 실행 중 발견된 문제다. 모든 오류를 `except:`로 숨기면 잘못된 결과가 계속 흘러갈 수 있다. **예상하고 처리할 수 있는 구체적인 예외만** 잡는다.
...
함수에 잘못된 입력이 들어왔을 때 조용히 엉뚱한 값을 반환하는 것보다 명확한 예외를 내는 편이 좋다.
...
- [ ] 실습 7문제 중 6문제 이상을 정답 없이 풀었다.
# 입문 5강. NumPy 첫걸음: 배열 생성, shape, dtype

## 1. 왜 필요한가

Python 리스트만으로도 숫자를 담을 수 있지만, 머신러닝은 수천~수백만 개 숫자를 같은 방식으로 계산한다. NumPy는 같은 자료형의 숫자를 다차원 배열에 저장하고 빠르게 **벡터화 연산**을 수행한다. pandas와 PyTorch도 shape와 dtype이라는 같은 언어를 사용하므로 NumPy가 데이터 분석의 다리 역할을 한다.
...
난수는 재현을 위해 generator seed를 고정한다. 같은 seed와 같은 NumPy 환경에서는 같은 순서의 난수를 얻을 수 있다.
...
Boolean 배열에서 `True`는 합계 계산 시 1처럼 취급된다. 조건에 맞는 원소를 선택하는 방식을 **Boolean indexing**이라 한다.
...
2. shape가 `(3, 2)`인 float32 1 배열을 만들라.
...
7. seed 7인 generator로 0 이상 100 미만 정수 12개를 `(3, 4)` shape로 만들고 shape와 평균을 출력하라.
...
핵심 예상 출력은 shape `(3, 4)`다. 같은 NumPy random generator에서 seed 7을 사용하면 값도 재현된다.
...
# 입문 6강. NumPy indexing, axis, reshape, broadcasting, copy

## 1. 왜 필요한가

머신러닝 오류의 상당수는 값 자체보다 shape와 축을 잘못 이해해 생긴다. "행별 평균"과 "열별 평균"을 뒤집거나, `(B,)`와 `(B, 1)`을 섞거나, slice가 원본을 공유한다는 사실을 놓치면 조용히 잘못된 결과가 만들어진다. 이번 강의는 배열을 안전하게 자르고 모양을 바꾸는 법을 다룬다.
...
`x[:, 1]`은 shape `(3,)`인 1차원 결과다. 열 차원을 유지하고 `(3, 1)`을 원한다면 `x[:, 1:2]`처럼 slice를 쓴다.
...
| 결합 시 dimension mismatch | 결합 축 밖의 크기가 다름 | 각 입력 shape 출력 |
...
4. shape `(5,)` 배열을 `(1, 5)`와 `(5, 1)`로 각각 바꾸라.
...
# 입문 7강. pandas 첫걸음: Series, DataFrame, 불러오기·점검·선택
...
`df.info()`는 열별 non-null 수와 dtype, 메모리 개요를 직접 출력한다. 반환값은 `None`이므로 `result = df.info()`에 분석 결과가 저장되는 것은 아니다.
...
pandas Series의 여러 조건도 각 조건을 괄호로 감싸고 `&`, `|`, `~`를 사용한다. `and`나 `or`를 쓰면 "Series의 truth value가 ambiguous"라는 오류가 난다.
...
3. 한 열을 Series와 `(B, 1)` DataFrame으로 각각 선택해 shape 차이를 출력하라.
...
# 입문 8강. pandas 정제: 결측·자료형·groupby·merge·concat

## 1. 왜 필요한가

실제 데이터는 비어 있는 값, 숫자처럼 보이는 문자열, 중복 행, 서로 다른 표에 나뉜 정보를 포함한다. 모델은 이러한 문제를 자동으로 이해하지 못한다. 정제 규칙을 명시하고, train에서 배운 통계만 validation/test에 적용하며, 표를 합친 뒤 행 수가 예상과 같은지 확인해야 데이터 누수를 피할 수 있다.
...
### 중요한 원칙: 대체값은 train에서만 계산

validation/test의 중앙값까지 함께 사용하면 미래 데이터를 미리 본 셈이 될 수 있다.
...
SQL join처럼 공통 key를 기준으로 정보를 붙인다.
...
`validate="one_to_one"`은 양쪽 key가 고유해야 한다는 계약을 검사한다. 다대일 결합이면 `many_to_one`을 쓸 수 있다. 결합 전후 행 수와 key 중복을 확인하지 않으면 예상치 못한 행 폭증이 생긴다.
...
4. 그룹별 행 수, value 평균, target 평균을 named aggregation으로 구하라.
...
- [ ] train 통계만 validation/test에 적용해야 하는 이유를 설명한다.
...
- 선, 막대, histogram, scatter, boxplot을 목적에 맞게 그린다.
...
예상 그래프: 세 범주의 막대 높이가 각각 2, 3, 1이다. 분류 문제에서 target 빈도는 가장 먼저 볼 그래프 중 하나다. 심한 불균형이면 accuracy만으로 평가하지 않을 수 있다.

## 4. 수치 분포: histogram과 boxplot

Histogram은 값을 구간(bin)으로 나눠 빈도를 보여 준다. bin 수가 지나치게 적거나 많으면 인상이 달라질 수 있으므로 여러 설정과 숫자 요약을 함께 본다.
...
상관은 인과관계를 증명하지 않는다. 또 Pearson 상관이 낮아도 곡선 관계가 있을 수 있고, 한 이상치가 상관을 크게 바꿀 수 있다. scatter와 숫자를 함께 본다.
...
1. **계약 확인**: 행 수, 열 이름, target, ID, 시간·그룹 열
2. **자료형**: 숫자가 문자열로 읽히지 않았는가?
3. **결측**: 열별 수와 비율, 특정 그룹에 몰렸는가?
4. **중복**: 업무 key 기준 중복이 있는가?
5. **target**: class 수, 비율, 이상한 label이 있는가?
6. **수치 분포**: 최솟값·최댓값·분위수·histogram·boxplot
7. **관계**: feature-target 그룹 비교, scatter, 상관
8. **split 위험**: 시간, 차량, 장비, 사용자 단위가 섞이면 누수인가?
9. **가설 기록**: 그래프 한 장당 "무엇을 확인했고 다음에 무엇을 할지" 한 문장
...
예상 출력은 x1, x2, x3 histogram 세 개다.
...
- target class 수가 너무 불균형해 accuracy가 오해를 만들지 않는가?
...
# 입문 10강. 미니 프로젝트: CSV → 정제 → NumPy → PyTorch Tensor

## 1. 왜 필요한가

지금까지 도구를 따로 배웠다. 이제 CSV 문자열을 읽고, 자료형과 결측을 정리하고, train 통계로 표준화한 뒤 NumPy와 PyTorch Tensor로 바꾼다. **이번 강의의 필수 종착점은 Tensor와 batch 확인이다.** 뒤의 신경망 학습은 선택 미리보기이며, 본과정 16~18강을 배운 후 돌아온다.

이 강의의 목적은 높은 점수가 아니다. 단계마다 **행 수, 열 순서, shape, dtype, NaN, 누수 여부**를 확인하는 습관을 만드는 것이다.

이 강의를 마치면 다음을 할 수 있다.

- 원시 표에서 feature와 target 계약을 정의한다.
- split을 먼저 만들고 train 통계만으로 결측 대체·표준화한다.
...
| feature | `speed`, `temperature`, `vibration` 세 수치 열 |
| target | 정상 0, 고장 1 |
| split | 앞 12행 train, 뒤 4행 validation인 교육용 고정 split |
| 결측 | train 중앙값으로 train·validation 모두 대체 |
| scaling | train 평균·표준편차로 표준화 |
| 모델 출력 | 샘플당 logit 하나, shape `[B, 1]` |
...
split을 만든 뒤에 train 중앙값·평균·표준편차를 계산해야 validation 정보가 preprocessing에 들어오지 않는다.

## 5. 3단계: train 통계로 결측 대체와 표준화
...
아래 함수는 학습된 통계를 딕셔너리로 반환하고, transform 함수는 그 통계를 새 데이터에 적용한다.
...
표준편차가 0인 열은 모든 값이 같아 나누기 0이 된다. 예제에서는 `replace(0.0, 1.0)`으로 안전하게 처리했다. 실전에서는 그 열이 의미 있는지 별도로 점검한다.
...
`torch.from_numpy`로 만든 CPU Tensor와 NumPy 배열은 메모리를 공유할 수 있다. 여기서는 DataFrame 변환 시 `copy=True`로 분석 표와의 관계를 명확히 했다. 이후에도 원본 보존이 중요하면 명시적으로 `.copy()` 또는 Tensor의 `.clone()`을 사용한다.
...
이 코드는 앞으로 배울 작업의 전체 모습이다. 지금은 암기·독립 재현을 요구하지 않는다. 준비가 되면 셀을 나눠 입력 shape부터 차례로 실행하자.
...
3. `optimizer.zero_grad()`: 이전 gradient를 비운다.
4. `logits = model(batch_x)`: 순전파한다.
5. `loss = loss_fn(logits, batch_y)`: 오차를 계산한다.
6. `loss.backward()`: 각 parameter의 gradient를 계산한다.
7. `optimizer.step()`: parameter를 업데이트한다.
...
- feature 열 순서가 train과 test에서 같은가?
- ID와 target이 feature에 섞이지 않았는가?
- 전처리 후 NaN·inf가 없는가?
- `X.shape[0] == y.shape[0]`인가?
- loss가 기대하는 target shape·dtype인가?
...
| loss에서 target size 불일치 | logits `[B,1]`, y `[B]` | y를 `reshape(-1, 1)` |
| Float/Double dtype 오류 | NumPy float64가 Tensor float64가 됨 | `to_numpy(dtype=np.float32)` |
| BCE loss 전에 sigmoid 적용 | `BCEWithLogitsLoss`가 logit을 기대 | 모델은 raw logit 출력 |
| validation 점수가 비현실적으로 높음 | 전체 데이터 통계·그룹 혼합 누수 | split 먼저, train 통계만 사용 |
...
5. `(5, 3)` Tensor와 `(5, 1)` target Tensor로 TensorDataset을 만들고 batch size 2일 때 모든 batch shape를 출력하라.
...
8. 5절 전처리 예제에서 feature 열 순서를 `['vibration','speed','temperature']`로 바꾸고 train과 validation에 동일하게 적용하라. 전체 학습 프로젝트는 실행하지 않아도 된다.
...
예상 train 중앙값은 x 2.0, y 15.0이며 validation 결측에도 그대로 적용된다.
...
- [ ] 필수 예제의 feature는 `(B, F)` float32, class label은 `(B,)` int64로 만든다. target 계약은 손실함수마다 다르며 `(B, 1)` float32 변환은 선택 8절에서 다룬다.
- [ ] DataLoader의 마지막 batch 크기가 작아질 수 있음을 안다.
- [ ] Tensor 변환 후 행 수·shape·dtype·NaN을 검사할 수 있다.
- [ ] 필수 실습 1~5, 8번을 풀고, 모르는 부분만 해설로 확인했다.

선택 심화(본과정 16–18강 이후): logit·확률·threshold 구분, 학습 루프, 예측 저장과 실습 6–7번. 지금 못해도 입문을 마친 것이다.

---

# 기초 10강을 마친 뒤: 기존 HDAT-DS 34강으로 넘어가는 법

입문 과정의 목적은 모든 Python 문법을 배우는 것이 아니다. 기존 강의에서 사용되는 데이터를 읽고, shape·dtype 오류를 찾고, 작은 함수를 직접 수정할 최소 기반을 만드는 것이다.
...
5. Tensor shape·dtype가 막히면 6강과 10강의 계약 표를 다시 본다.
6. 한 오류에 20분 이상 갇히면 오류 마지막 줄, 입력 shape·dtype, 최소 재현 코드 순서로 문제를 줄인다.
...
- [ ] train 통계만 validation/test에 적용하는 이유를 안다.

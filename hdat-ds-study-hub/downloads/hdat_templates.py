...
class TabularConfig:
    ...
    seed: int = 42
    output_kind: Literal["value", "label", "probability"] | None = None  # 명시 권장
    positive_label: object = 1  # binary 확률의 의미. EDIT
    class_order: tuple | None = None  # multiclass 확률 열 순서. EDIT
    ...
    if metric in {"auc", "roc_auc"}:
        ...
        if y_true_arr.ndim == 2:
            if not set(np.unique(y_true_arr)).issubset({0, 1}):
                raise ValueError("다중출력 AUC helper는 0/1 multilabel만 지원합니다.")
        ...
        if score_arr.ndim == 2:
            score_arr = score_arr[:, 0 if score_arr.shape[1] == 1 else 1]
...
def fit_tabular_baseline(
    ...
    if output_kind is None:
        if cfg.metric.lower() in {"auc", "roc_auc"}:
            raise ValueError("AUC여도 제출 규격은 별도입니다. output_kind를 'label' 또는 'probability'로 명시하세요.")
        output_kind = "value" if cfg.task == "regression" else "label"
    if output_kind == "probability":
        test_pred = predict_tabular_probabilities(
            pipe, X_test, cfg.task, cfg.positive_label, cfg.class_order
        )
    elif output_kind == ("value" if cfg.task == "regression" else "label"):
...
def predict_tabular_probabilities(pipe, X, task, positive_label=1, class_order=None):
    """metric이 아닌 제출 계약에 맞춰 확률을 선택한다."""
    probabilities = pipe.predict_proba(X)
    classes = pipe.named_steps["model"].classes_
    if task == "binary":
        return probabilities[:, list(classes).index(positive_label)]
    if task == "multiclass":
        order = list(classes) if class_order is None else list(class_order)
        if len(order) != len(classes) or set(order) != set(classes):
    ...
    if task == "multilabel":
        ...
        for probability, labels in zip(probabilities, classes):
            if not set(labels).issubset({0, 1}):
                raise ValueError("multilabel은 0/1 indicator만 지원합니다.")
            columns.append(probability[:, list(labels).index(1)] if 1 in labels else np.zeros(len(X)))
        return np.column_stack(columns)
    raise ValueError("회귀에는 확률 제출을 사용할 수 없습니다.")
...
def split_raw_time_then_window(
    ...
    gap: int = 0,
    assume_sorted: bool = False,
    split_mode: Literal["forecast_origin", "target"] = "forecast_origin",
    label_delay: int = 0,
):
    """미래 예측은 train label이 validation 예측 기준시점에 알려져 있어야 한다.

    기본 forecast_origin: train target+label_delay < cut-gap,
    valid input 마지막 시점 >= cut+gap. target 모드는 별도 배치평가 명세용이다.
    label_delay/gap은 같은 시간순 행 단위다. 불규칙 시각은 실제 timestamp로 검사한다.
    """
    if not assume_sorted:
        raise ValueError("시간순 stable sort를 확인한 뒤 assume_sorted=True로 호출하세요.")
    n = len(features)
    if n < 2 or not 0 < train_ratio < 1 or gap < 0 or label_delay < 0:
    ...
    train_mask = target_idx + label_delay < max(cut - gap, 0)
    if split_mode == "forecast_origin":
        valid_mask = target_idx - horizon >= cut + gap
    elif split_mode == "target":
        valid_mask = target_idx >= cut + gap
    else:
        raise ValueError("split_mode는 forecast_origin 또는 target입니다.")
...
if torch is not None:
    ...
    def _batch_loss(model, xb, yb, criterion, task, device):
        ...
        # CE(weight=...)의 mean 분모는 batch 크기가 아니라 정답 class weight 합이다.
        denominator = len(xb)
        if isinstance(criterion, nn.CrossEntropyLoss):
            keep = yb != criterion.ignore_index
            denominator = (criterion.weight[yb[keep]].sum().item()
                           if criterion.weight is not None else keep.sum().item())
        if denominator <= 0:
            raise ValueError("loss 평균의 분모가 0입니다. class weight/ignore_index를 확인하세요.")
        return criterion(logits, yb), denominator
    ...
    def train_torch_model(
        ...
        직접 수행한다. maximize=True는 AUC/F1/Accuracy, False는 오차 지표.
        loss_fn은 mean reduction만 지원하며 가중 CE는 hard class index만 지원한다.
        VAE/GAN·가변길이 mask 입력은 전용 학습 루프가 필요하다.
        """
        ...
        if getattr(criterion, "reduction", "mean") != "mean":
            raise ValueError("공통 루프는 reduction='mean' loss만 지원합니다.")
        if maximize and score_fn is None:
            raise ValueError("loss는 최소화합니다. maximize=True에는 score_fn이 필요합니다.")
        ...
        for epoch in range(1, epochs + 1):
            ...
            score = valid_loss if score_fn is None else float(score_fn(
                np.concatenate(score_targets), np.concatenate(score_outputs)))
            if not np.isfinite(score):
                raise ValueError("validation metric이 NaN/Inf입니다. class·shape·복원을 확인하세요.")
            history.append({"epoch": epoch, "train_loss": train_loss,
                            "valid_loss": valid_loss, "monitor": score})
            print(f"epoch={epoch:03d} train={train_loss:.6f} valid={valid_loss:.6f} monitor={score:.6f}")

            if (score > best_score + 1e-8) if maximize else (score < best_score - 1e-8):
                best_score = score
                best_state = {
